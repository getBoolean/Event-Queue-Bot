...
export const GUILD_TABLE = sqliteTable("guild", ({
	...
	messagesReceived: integer("messages_received").$type<bigint>().notNull().default(0n),
	commandsReceived: integer("commands_received").$type<bigint>().notNull().default(0n),
	buttonsReceived: integer("buttons_received").$type<bigint>().notNull().default(0n),
	queuesAdded: integer("queues_added").$type<bigint>().notNull().default(0n),
	voicesAdded: integer("voices_added").$type<bigint>().notNull().default(0n),
	displaysAdded: integer("displays_added").$type<bigint>().notNull().default(0n),
	membersAdded: integer("members_added").$type<bigint>().notNull().default(0n),
	schedulesAdded: integer("schedules_added").$type<bigint>().notNull().default(0n),
	whitelistedAdded: integer("whitelisted_added").$type<bigint>().notNull().default(0n),
	blacklistedAdded: integer("blacklisted_added").$type<bigint>().notNull().default(0n),
	prioritizedAdded: integer("prioritized_added").$type<bigint>().notNull().default(0n),
	adminsAdded: integer("admins_added").$type<bigint>().notNull().default(0n),
	archivedMembersAdded: integer("archived_members_added").$type<bigint>().notNull().default(0n),
}));
...
export const PRIORITIZED_TABLE = sqliteTable("prioritized", ({
	...
	subjectId: text("subject_id").$type<Snowflake>().notNull(),
	isRole: integer("is_role", { mode: "boolean" }).notNull(),
	priorityOrder: integer("priority_order").$type<bigint>().notNull().default(5n),
	reason: text("reason"),
}),
(table) => ({
	unq: unique().on(table.queueId, table.subjectId),

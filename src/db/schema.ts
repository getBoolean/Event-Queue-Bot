...
export const GUILD_TABLE = sqliteTable("guild", ({
	...
	messagesReceived: integer("messages_received").$type<bigint>().notNull().default(0),
	commandsReceived: integer("commands_received").$type<bigint>().notNull().default(0),
	buttonsReceived: integer("buttons_received").$type<bigint>().notNull().default(0),
	queuesAdded: integer("queues_added").$type<bigint>().notNull().default(0),
	voicesAdded: integer("voices_added").$type<bigint>().notNull().default(0),
	displaysAdded: integer("displays_added").$type<bigint>().notNull().default(0),
	membersAdded: integer("members_added").$type<bigint>().notNull().default(0),
	schedulesAdded: integer("schedules_added").$type<bigint>().notNull().default(0),
	whitelistedAdded: integer("whitelisted_added").$type<bigint>().notNull().default(0),
	blacklistedAdded: integer("blacklisted_added").$type<bigint>().notNull().default(0),
	prioritizedAdded: integer("prioritized_added").$type<bigint>().notNull().default(0),
	adminsAdded: integer("admins_added").$type<bigint>().notNull().default(0),
	archivedMembersAdded: integer("archived_members_added").$type<bigint>().notNull().default(0),
	eventsAdded: integer("events_added").$type<bigint>().notNull().default(0),
}));
...
export const QUEUE_TABLE = sqliteTable("queue", ({
	...
	pullBatchSize: integer("pull_batch_size").$type<bigint>().notNull().default(1),
	...
	rejoinCooldownPeriod: integer("rejoin_cooldown_period").$type<bigint>().notNull().default(0),
	rejoinGracePeriod: integer("rejoin_grace_period").$type<bigint>().notNull().default(0),
...
export const PRIORITIZED_TABLE = sqliteTable("prioritized", ({
	...
	subjectId: text("subject_id").$type<Snowflake>().notNull(),
	isRole: integer("is_role", { mode: "boolean" }).notNull(),
	priorityOrder: integer("priority_order").$type<bigint>().notNull().default(5),
	reason: text("reason"),
}),
(table) => ({
	unq: unique().on(table.queueId, table.subjectId),
